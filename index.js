...
const SYS_TABLES = {
    arm64_linux4: syscalls
}

const systool = {
    syscalls: SYS_TABLES.arm64_linux4,
    /**
     * To get syscall info from interruption number
     * @param pNum
     * @return {null}
     */
    interruptLookup: function( pNum){
        let s = null;
        for(let i=0; i<SYS_TABLES.arm64_linux4.length; i++){
            if(SYS_TABLES.arm64_linux4[i][2]===pNum){
                s = SYS_TABLES.arm64_linux4[i];
                continue;
            }
        }
        return s;
